...
bathymetry_2d, h_viscosity, elev_init, uv_init = tools.thetis_support_scripts.initialise_fields(mesh2d, inputdir,
...
lagoon_input = input_barrages.input_predefined_barrage_specs(N_t, N_s, operation=operation,
                                                             turbine_parameters=turbine_params)
lagoon_status = pickle.load(open(inputdir + "/barrage_status_" + str(identifier) + ".p", "rb"))

# Set initial energy output to 0.
...
det_xy, det_names = tools.detectors.get_detectors(mesh2d, maximum_distance=max_dist)                               #tidal gauge detectors
extra_detectors, extra_detector_names = select_and_move_detectors(mesh2d, extra_detectors, extra_detector_names,   #extra detectors
                                                                  maximum_distance=100.)

cb = DetectorsCallback(solver_obj, det_xy, ['elev_2d', 'uv_2d'], name='detectors_gauges',
                       detector_names=det_names)
cb_2 = DetectorsCallback(solver_obj, extra_detectors, ['elev_2d', 'uv_2d'], name='detectors_TRS',
                         detector_names=extra_detector_names)

# Adding callback objects to solver
solver_obj.add_callback(cb, 'timestep')
solver_obj.add_callback(cb_2, 'timestep')
...
for lag in lagoons:
    cb_lagoon = LagoonCallback(solver_obj, {"inner": dx(lagoon_bnd[lag]['inner_id']), "outer": dx(lagoon_bnd[lag]['outer_id'])}, lagoon_input,
                               ...
                               adaptive_control=None, export_to_hdf5=True)

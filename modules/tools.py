...
from numpy import array, empty, zeros, append, average, delete
...
class LagoonCallback(DiagnosticCallback):
    ...
    def __init__(self, solver_obj, marked_areas, lagoon_input, name="lagoon", thetis_boundaries=None,
        ...
        self.elevation_array = {"inner": empty(number_timesteps),
                                "outer": empty(number_timesteps)}
        ...
        field_dims = [12]
        attrs = {'field_names': array(["operation_output"], dtype='S'),
    ...
    def __call__(self):
        ...
        for i in ["inner", "outer"]:
            ...
            self.elevation_array[i] = append(delete(self.elevation_array[i], [0]), self.elevation_value[i])
            self.elevation_mean[i] = average(self.elevation_array[i])

        if self.adaptive_control is not None:
            self.control = self.adaptive_control[int(self.time_operation / (12.42 * 3600))]
            if self.holding_time is not None:
                if self.status["m"] == 1 and self.status["m_t"] + 4.0 > \
                        self.holding_time[int(self.time_operation / (12.42 * 3600))][0] >= self.status["m_t"]:
                    self.control["h_t"][0] = (self.holding_time[int(self.time_operation/ (12.42 * 3600))][0] -
                                              self.status["m_t"] + 2 * self.control["h_t"][0]) / 3
                elif self.status["m"] == 6 and self.status["m_t"] + 4.0 >\
                        self.holding_time[int(self.time_operation / (12.42 * 3600))][1] >= self.status["m_t"]:
                    self.control["h_t"][1] = (self.holding_time[int(self.time_operation/ (12.42 * 3600))][1] -
                                              self.status["m_t"] + 2 * self.control["h_t"][1]) / 3

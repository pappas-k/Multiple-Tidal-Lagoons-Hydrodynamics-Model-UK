...
def initialise_barrage(Barrage_Number=1, time=0.):
    ...
    for i in range(Barrage_Number):
        ...
        barrage_status.append({"m": Mode, "m_t": time/3600, "m_dt": ModeDuration, "DZ": DZ, "f_r": rampf, "Q_t": QTurbines,
...
def input_predefined_barrage_specs(NumTB, NumSL, operation='two-way', turbine_parameters=None):
    ...
    if operation == "ebb":
        input_2D.append({"h_t": [3.5, 0.], "h_p": 1.5, "t_p": [0., 0.], "g_t": [6.0, 6.0], "tr_l": [7, -6],
                         "N_t": NumTB, "N_s": NumSL})
    elif operation == "ebb-pump":
        input_2D.append({"h_t": [3.5, 0.], "h_p": 1.5, "t_p": [1.0, 0.], "g_t": [6.0, 6.0], "tr_l": [7, -6],
                         "N_t": NumTB, "N_s": NumSL})
    elif operation == "two-way":
        input_2D.append({"h_t": [2.7, 2.7], "h_p": 1.5, "t_p": [0., 0.], "g_t": [2.0, 2.0], "tr_l": [7, -6],
                         "N_t": NumTB, "N_s": NumSL})
    elif operation == "two-way-pump":
        input_2D.append({"h_t": [3., 3.], "h_p": 2.0, "t_p": [1.0, 0.5], "g_t": [2.0, 2.0], "tr_l": [7, -6],
                         "N_t": NumTB, "N_s": NumSL})
...
def input_predefined_system_specs(NumTB, NumSL):
    ...
    turbine_params = {"f_g": 50, "g_p": 95, "g": 9.807, "t_d": 7.35,
                      ...
                      "eta": [0.95, 0.95], "options": 0}

    Coed_t = turbine_parametrisation(turbine_params["h_min"],
                                     turbine_params)[1] / ((math.pi * (turbine_params["t_d"] / 2)**2) *
                                                           math.sqrt(2 * turbine_params["g"] * turbine_params["h_min"]))

    sluice_params = {"a_s": 180, "c_d": 1.0, "c_t": Coed_t, "g": turbine_params["g"]}

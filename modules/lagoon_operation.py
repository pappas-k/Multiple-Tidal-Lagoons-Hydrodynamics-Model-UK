import math
import numpy as np
from modules.parameterisations import turbine_parametrisation, turbine_generation, gate_sluicing, turbine_sluicing, \
    ramping_turbine
...
def tidal_lagoon_0d_model(simulation, elev_time, area_elev, status, control, params, export_output=None,
    ...
    for step in range(0, int(simulation["t"] / simulation["Dt"]), 1):
        ...
        if variable_tide_limits is not None:
            control["tr_l"][0] = variable_tide_limits[0](step * simulation["Dt"]+simulation["start_t"])
            control["tr_l"][1] = variable_tide_limits[1](step * simulation["Dt"]+simulation["start_t"])
